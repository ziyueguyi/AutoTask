    ...
    def get_user_info(self):
        ...
        if response.status_code == 200:
            if response.json().get('error_code') == 0:
                current_value = response.json().get('level_info').get('current_value')
                current_level = response.json().get('level_info').get('current_level')
                self.initialize.info_message(f"当前等级: {current_level}", is_flag=True)
                self.initialize.info_message(f"当前经验: {current_value}", is_flag=True)
                return True
            elif response.json().get('error_code') == 36003:
